...
class RNNTriplet(nn.Module):
    def __init__(self, device, device_str, input_vocab_size: int, embedding_size=128, hidden_size=256, dropout_rate=0.1, nl=4, *args, **kwargs):
        super(RNNTriplet, self).__init__(*args, **kwargs)
        ...
        self.expansion_pitch = nn.Linear(hidden_size, input_vocab_size)
        self.expansion_step = nn.Linear(hidden_size, 1)
        self.expansion_duration = nn.Linear(hidden_size, 1)
    ...
    def forward(self, x):
        ...
        output_pitch = self.expansion_pitch(output)
        output_pitch = F.log_softmax(output_pitch, dim=1)
        output_step = self.expansion_step(output)
        output_step = F.relu(output_step)
        output_duration = self.expansion_duration(output)
        output_duration = F.relu(output_duration)
        return output_pitch, output_step, output_duration

    def train_model(self, train_batches, val_batches, epochs: int, optimizer, criterion, scaler, criterion_unit = nn.MSELoss()):
        train_losses = []
        accuracies = []
        val_losses = []
        ...
        for ep in tqdm(range(epochs)):
            ...
            for batch in tqdm(train_batches, leave=False):
                ...
                with autocast(self.device_str):
                    output_tensor, output_step, output_duration = self(input_tensor)
                    # TODO: check shapes
                    loss_pitch = criterion(output_tensor.view(-1, output_tensor.size(-1)), label_tensor.view(-1))
                    loss_step = criterion_unit(output_step, label_tensor)
                    loss_duration = criterion_unit(output_duration, label_tensor)
